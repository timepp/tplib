...
#define MENUID_COPY               1001
#define MENUID_PASTE              1002
#define MENUID_PASTE_AS_CHILD     1003
#define MENUID_CUT                1004
#define MENUID_DELETE             1005
#define MENUID_ADD_COMP_BEGIN     1100
#define MENUID_ADD_COMP_END       1199
#define MENUID_CVT_COMP_BEGIN     1200
#define MENUID_CVT_COMP_END       1299
#define MENUID_ADD_COMP_AS_PARENT_BEGIN 1300
#define MENUID_ADD_COMP_AS_PARENT_END   1399
...
public:
	...
	CCompositeTreeCtrl(): m_clipboard(NULL), m_fc(NULL), m_lb_down(false), m_dragging(false)
	{
	}
	~CCompositeTreeCtrl()
	{
...
private:
	...
	void PopupContextMenu(CPoint pt)
	{
		...
		CMenu mu;
		mu.CreatePopupMenu();
		mu.AppendMenu(MF_POPUP, muAdd.m_hMenu, L"添加(&A)");
		mu.AppendMenu(MF_POPUP, muAddAsParent.m_hMenu, L"添加父节点(&Q)");
		mu.AppendMenu(MF_POPUP, muCvt.m_hMenu, L"切换为(&S)");
		mu.AppendMenu(MF_SEPARATOR);
		mu.AppendMenu(MF_STRING, MENUID_COPY, L"复制(&C)\tCtrl+C");
		mu.AppendMenu(MF_STRING, MENUID_CUT, L"剪切(&X)\tCtrl+X");
		mu.AppendMenu(MF_STRING, MENUID_PASTE, L"粘贴(&V)\tCtrl+V");
		mu.AppendMenu(MF_STRING, MENUID_PASTE_AS_CHILD, L"粘贴为子结点(&B)");
		mu.AppendMenu(MF_STRING, MENUID_DELETE, L"删除(&D)\tDel");
	...
	{
		bool bControlKeyIsDown = KEY_IS_DOWN(VK_CONTROL);
		bool bShiftKeyIsDown = KEY_IS_DOWN(VK_SHIFT);
		if (ch == VK_DELETE)
		{
	...
	void OnLButtonUp(UINT /*code*/, CPoint /*pt*/)
	{
		...
		if (m_dragging)
		{
			...
			{
				DragComponent(m_dragging_source, m_dragging_target, KEY_IS_DOWN(VK_CONTROL));
			}
		}
		m_dragging = false;
